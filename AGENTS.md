...
- `pi.py` — the BE02 PI algorithm (`pi()`, `cape()`, entropy solver), the whole-field fast path `pi_field()` (parallel guvectorize; ~2 orders of magnitude over per-column loops), and the Wing et al. (2015) log-decomposition `pi_log_decomposition()`. Numba-compiled. `cape()` is a thin wrapper over `_cape_env_setup` (per-column environment work: NaN scan, ptop slice, ordering check, environmental density temperatures — hoisted because the environment is identical across the ~20 CAPE calls of a column) + `_cape_core` (the parcel-dependent math, line-for-line the original algorithm).
